...
window.onload = () => {
  ...
  const venCharacterCacheBust = "ven-preserve-rig-001";
  const characterOptions = Object.freeze([
    ...
    },
    {
      ...
      resourcePath: "assets/characters/character-03/",
  ...
  function applyBasicUserSettings(settings) {
    ...
    if (settings.appearance?.selectedCharacterId !== undefined) {
      setSelectedPlayableCharacter(settings.appearance.selectedCharacterId, {
        persist: false
      });
  ...
  function getSelectedCharacterModelUrl() {
  ...
  function getSelectedCharacterResourceUrl() {
    const characterConfig = getActivePlayableCharacterConfig();
  ...
  function getSelectedCharacterAnimationUrl(animationKey) {
    const characterConfig = getActivePlayableCharacterConfig();
  ...
  function loadMotusManTemplate() {
    ...
    if (motusManTemplate && motusManTemplateCharacterId === characterId) {
      return Promise.resolve(motusManTemplate);
    }

    if (motusManLoadPromise && motusManLoadPromiseCharacterId === characterId) {
    ...
    loader.setResourcePath(getSelectedCharacterResourceUrl());
    motusManLoadPromiseCharacterId = characterId;
    motusManLoadPromise = new Promise((resolve, reject) => {
      const modelUrl = getSelectedCharacterModelUrl();
      if (isMotusManConfig(characterConfig)) {
  ...
  function loadMotusManIdleClip() {
    ...
    motusManIdleLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("standAimIdle"),
        (fbx) => {
  ...
  function loadMotusManCrouchIdleClip() {
    ...
    motusManCrouchIdleLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("crouchAim"),
        (fbx) => {
          const clip = fbx.animations?.[0];
          if (!clip) {
  ...
  function loadMotusManWalkAimClip() {
    ...
    motusManWalkAimLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("walkAim"),
        (fbx) => {
          const clip = fbx.animations?.[0];
          if (!clip) {
  ...
  function loadMotusManJogAimClip() {
    ...
    motusManJogAimLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("jogAim"),
        (fbx) => {
          const clip = fbx.animations?.[0];
          if (!clip) {
  ...
  function loadMotusManJumpStartClip() {
    ...
    motusManJumpStartLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("jumpStart"),
        (fbx) => {
          const clip = fbx.animations?.[0];
          if (!clip) {
  ...
  function loadMotusManJumpAirClip() {
    ...
    motusManJumpAirLoadPromise = new Promise((resolve, reject) => {
      loader.load(
        getSelectedCharacterAnimationUrl("jumpAir"),
        (fbx) => {
          const clip = fbx.animations?.[0];
          if (!clip) {
  ...
  function preparePlayerCharacterAnimationClips(modelRoot, characterConfig, clips) {
    ...
    if (totalRotationTracks > 0 && rotationMatchRatio < 0.5) {
      console.warn("[CHARACTER] Character 02 animation still disabled: too few safe bone matches.", {
  ...
  async function runStartupSequence() {
    ...
    try {
      setStartupPhase("Loading character previews", "Preloading character selector images.");
      const characterPreviewReadyTask = preloadCharacterPreviewImages()
        .catch((error) => {
          console.warn("[CHARACTER PREVIEW PRELOAD] preload finished with errors", error);
        })
        .then(() => {
          setStartupReadiness("characterPreviewsReady", true, {
            statusMessage: "Loading character previews",
            debugMessage: "Character selector preview images attempted.",
            logPhase: "character previews ready"
          });
          syncCharacterSelectionUi();
        });
      ...
      await Promise.all([characterPreviewReadyTask, playerReadyTask, mapReadyTask]);
  ...
  function syncCharacterPreviewImage(selectedCharacter) {
    ...
    if (cachedPreview?.loaded) {
      ...
      if (characterPreviewImage.getAttribute("src") !== previewPath) {
        characterPreviewImage.src = previewPath;
      }
    ...
    loadCharacterPreviewImage(previewPath).then((result) => {
      if (characterPreviewImage.dataset.previewPath !== previewPath) {
        return;
      }
      if (result.loaded) {
        characterPreviewImage.onload = null;
  ...
  function applySelectedPlayableCharacter({ showStatus = true } = {}) {
    const selectedCharacter = getSelectedPlayableCharacterConfig();
    activePlayableCharacterId = selectedCharacter.id;
    console.log("[CHARACTER] Applied character:", selectedCharacter.name, selectedCharacter.modelPath, selectedCharacter.animationFolder);
    ...
    reloadLocalPlayerCharacterVisual("Apply Character clicked")
      .catch((error) => {
        console.error("[CHARACTER] Failed to apply character to local player", error);
      });
  }
